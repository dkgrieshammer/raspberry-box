...
#        print "Card read UID: "+uid
        if lastcarduid:
            if lastcarduid != uid or (time.clock() - lastcardtime >= CARDTIMEOUT):
                # new card faster than we checked or timeout
                print '{ "cardUID": "'+uid+'","event":"added" }'
                lastcarduid = uid
        else:
            print '{ "cardUID": "'+uid+'","event":"added" }'
            lastcarduid = uid
    ...
    elif time.clock() - lastcardtime >= CARDTIMEOUT:
        # status not ok
        if lastcarduid:
            print '{ "cardUID": "'+uid+'","event":"removed" }'
            lastcarduid = None
        # else
            # do nothing -> pass
